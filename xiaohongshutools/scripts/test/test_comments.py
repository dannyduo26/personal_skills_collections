...
async def test_comments():
    ...
    # Create session
    # 创建 XHS 会话
    xhs = await create_xhs_session(proxy=None)
    
    # Manually inject all user cookies to ensure session consistency
    # 手动注入所有 Cookie 以保持会话一致性，避免 461 错误
    ...
    try:
        # Fetch comments
        # 获取笔记评论
        print(f"Fetching comments for note_id: {note_id}...")
        res_comments = await xhs.apis.comments.get_comments(note_id, xsec_token)
        comments_data = await res_comments.json()
        ...
        if comments_data.get("code") == 0:
            ...
            for comment in comments:
                ...
                print(f"[{user}]: {content} (Likes: {likes})")
        else:
            print(f"Error fetching comments: {comments_data}")
