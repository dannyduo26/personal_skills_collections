...
```bash
python3 <SKILL_DIR>/scripts/fetch.py --url "<url>" --code "<code>" [--max_chars <int>] [--stealth] [key=value ...]
```
...
| Domain | Command | Why |
|--------|---------|-----|
| `mp.weixin.qq.com` | `fetch.py --url <url> --code wechat --stealth` | JS-rendered content |
| `zhuanlan.zhihu.com` | `fetch.py --url <url> --code zhihu --stealth` | Anti-scraping + JS |
| `juejin.cn` | `fetch.py --url <url> --code juejin --stealth` | JS-rendered SPA |
| `sspai.com` | `fetch.py --url <url> --code sspai` | Static HTML |
| `blog.csdn.net` | `fetch.py --url <url> --code csdn` | Static HTML |
| `ruanyifeng.com` | `fetch.py --url <url> --code blog` | Static blog |
| `openai.com` | `fetch.py --url <url> --code openai` | Static HTML |
| `blog.google` | `fetch.py --url <url> --code google` | Static HTML |
| Everything else | `fetch.py --url <url> --code auto` | Auto-fallback handles it |

## Script Options

```bash
# Basic — auto-selects fast or stealth. Output is JSON by default.
python3 <SKILL_DIR>/scripts/fetch.py --url "https://sspai.com/post/73145" --code "sspai_test"

# Force stealth for known JS-heavy sites
python3 <SKILL_DIR>/scripts/fetch.py --url "https://mp.weixin.qq.com/s/xxx" --code "wechat_test" --stealth

# Limit output to 15000 characters and add custom metadata
python3 <SKILL_DIR>/scripts/fetch.py --url "https://example.com" --code "test" --max_chars 15000 author=John

# Results are also saved to <SKILL_DIR>/json/test_YYYY-MM-DD_HH-mm-ss.json
```
