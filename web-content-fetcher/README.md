...
```bash
# 基础用法 (默认输出 JSON，并保存到 json/ 目录)
python3 scripts/fetch.py --url https://sspai.com/post/73145 --code sspai_test

# 强制 stealth 模式
python3 scripts/fetch.py --url https://mp.weixin.qq.com/s/xxx --code wechat_test --stealth

# 限制输出字符数并添加自定义元数据
python3 scripts/fetch.py --url https://example.com --code test --max_chars 15000 author=John
```
...
| 域名 | 模式 | 示例命令 |
|------|------|----------|
| `mp.weixin.qq.com` | `--stealth` | `fetch.py --url <url> --code wechat --stealth` |
| `zhuanlan.zhihu.com` | `--stealth` | `fetch.py --url <url> --code zhihu --stealth` |
| `juejin.cn` | `--stealth` | `fetch.py --url <url> --code juejin --stealth` |
| `sspai.com` | fast | `fetch.py --url <url> --code sspai` |
| 其他 | fast | 自动降级 |
